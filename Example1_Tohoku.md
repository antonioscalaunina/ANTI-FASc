...
Finally with the following commands you run the slip distributions computation:
 ...
 They can be easily plotted by simple personal scripts. In the folder *utils* there is the script *slip_distribution_plot_AGI.m*. As for all the MATLAB script, this one can be run either in the MATLAB command window or with one of the following commands depending:
